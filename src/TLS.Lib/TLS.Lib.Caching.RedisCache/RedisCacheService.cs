...
using AEC.Core.Caching.Redis;

namespace AEC.Lib.Caching.RedisCache
{
    public class RedisCacheService : IRedisCacheService
    {
        ...
        public RedisCacheService(IRedisCacheSerializer serializer)
        {
            if (serializer == null)
            {
        ...
        {
            Connect(connectionString, null);
        }

        public void Connect(string connectionString, int? database)
        {
            ...
            this.connectionMultiplexer = ConnectionMultiplexer.Connect(connectionString);
            db = connectionMultiplexer.GetDatabase(database.Value);
        }
        ...
        /// serializer
        /// </exception>
        //public RedisCacheClient(ConnectionMultiplexer connectionMultiplexer, ICacheSerializer serializer, int database = 0)
        //{
        ...
        /// The serializer.
        /// </value>
        public IRedisCacheSerializer Serializer
        {
        ...
        public T Get<T>(string key, Func<T> getFunc) where T : class
        {
            return GetInternal(key, getFunc, null, null);
        }
        public T Get<T>(string key, Func<T> getFunc, DateTimeOffset expiresAt) where T : class
        {
            return GetInternal(key, getFunc, expiresAt, null);
        }
        public T Get<T>(string key, Func<T> getFunc, TimeSpan expiresIn) where T : class
        {
            return GetInternal(key, getFunc, null, expiresIn);
        }
        private T GetInternal<T>(string key, Func<T> getFunc, DateTimeOffset? expiresAt, TimeSpan? expiresIn) where T : class
        {
            var cached = Get<T>(key);
            if (cached == default(T) && getFunc != null)
            {
                cached = getFunc();
                if (cached != default(T))
                {
                    ...
                    {
                        success = Add(key, cached, expiresAt.Value);
                    }
                    else if (expiresIn.HasValue)
                    {
                        success = Add(key, cached, expiresIn.Value);
                    }
                    else
                    {
                        success = Add(key, cached);
                    }
                    if (!success)
                    {
                        //throw new Exception($"Can not add object value of [{typeof(T).FullName}] to cache");
                    }
                }
            }
            return cached;
        ...
        public async Task<T> GetAsync<T>(string key, Func<T> getFunc) where T : class
        {
            return await GetAsyncInternal(key, getFunc, null, null);
        }
        public async Task<T> GetAsync<T>(string key, Func<T> getFunc, DateTimeOffset expiresAt) where T : class
        {
            return await GetAsyncInternal(key, getFunc, expiresAt, null);
        }
        public async Task<T> GetAsync<T>(string key, Func<T> getFunc, TimeSpan expiresIn) where T : class
        {
            return await GetAsyncInternal(key, getFunc, null, expiresIn);
        }
        private async Task<T> GetAsyncInternal<T>(string key, Func<T> getFunc, DateTimeOffset? expiresAt, TimeSpan? expiresIn) where T : class
        {
            ...
            if (cached == default(T) && getFunc != null)
            {
                ...
                if (cached != default(T))
                {
                    ...
                    if (expiresAt.HasValue)
                    {
                        success = await AddAsync(key, cached, expiresAt.Value);
                    }
                    else if (expiresIn.HasValue)
                    {
                        success = await AddAsync(key, cached, expiresIn.Value);
                    }
                    else
                    {
                        success = await AddAsync(key, cached);
                    }
                    if (!success)
                    {
                        //throw new Exception($"Can not add object value of [{typeof(T).FullName}] to cache");
        ...
        public async Task<List<T>> GetAllAsync<T>(List<string> keys) where T : class
        {
            ...
            return data.Where(item => !item.IsNull).Select(redisValue => serializer.Deserialize<T>(encoding.GetBytes(redisValue.ToString()))).ToList();
        }
        ...
        {
            await SaveAsync(SaveType.BackgroundSave);
        }
